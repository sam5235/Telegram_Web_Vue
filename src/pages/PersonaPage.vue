...
import { usePersona } from 'stores/persona-store';
import { storeToRefs } from 'pinia';
import PersonaCard from 'components/PersonaCard.vue';
import { useRouter } from 'vue-router';
import { useTelegram } from 'src/stores/telegram';
import { useChat } from 'src/stores/chat-store';
import { get_all_persona_types } from 'src/apis/persona';

const isScrollEnd = ref<boolean>(false);
const searchQuery = ref<string>('');
const router = useRouter();

const { showBack, theme_data } = useTelegram();
const { personas, meta_data, selected_persona_type, is_loading } = storeToRefs(
  usePersona()
);
const {
  reset,
...
const { all_persona_types } = storeToRefs(usePersona())

const { ResetChat } = useChat();
const { persona_id: persona_id_store } = storeToRefs(useChat());

async function onLoad(index: number, done: () => void) {
  // fetch and set personas
  await searchPaginate(searchQuery.value, selected_persona_type.value).then(
    () => {
      done();
      if (
...
const personaClicked = (
  persona_id: string,
  full_name: string,
  persona_image: string
) => {
  ResetChat();
  persona_id_store.value = persona_id;
...
async function onFilter(persona_type: string) {
  selected_persona_type.value = persona_type;
  await reset();
  isScrollEnd.value = false;
  await searchPersonas(searchQuery.value, persona_type);
}

onMounted(async () => {
  showBack()
  await get_all_persona_types()
    .then((response: any) => {
      console.log(response.data)

      response.data?.forEach((persona_type: string) => {
        all_persona_types.value.push(persona_type);
      });
    })
    .catch((error: any) => {
      console.log(error);
  ...
  await reset();
  isScrollEnd.value = false;
  await searchPersonas(searchQuery.value, 'All');
  console.log('persona page mounted');
});
</script>
<template>
  <q-page class="bg-secondary">
      ...
      <div class="header-section text-info">
        ...
        <div class="toolbar-container">
          ...
          <form>
            ...
            />
            <q-btn
              @click="searchPersonas(searchQuery, selected_persona_type)"
              icon="search"
              color="accent"
              flat
              rounded
        ...
        >
          <div
            ...
            v-for="persona_type in all_persona_types"
            :key="persona_type"
            :aria-current="persona_type === selected_persona_type"
            @click="onFilter(persona_type)"
            :class="
              persona_type === selected_persona_type
                ? 'bg-accent text-info'
    ...
    >
      <PersonaCard
        ...
        :personas="personas"
        :persona-clicked="personaClicked"
      />

      <div class="no-result" v-else-if="personas.length == 0 && !is_loading">
        <svg
          width="246"
          height="210"
          viewBox="0 0 246 210"
